...
function SalarySetModal({
    ...
    function handleComplete(e){
        ...
        validateFieldsAndScroll((errors,values) => {
            ...
            let array = setSalaryCourseCommission;
            for(let i in array){
                if(array[i].royaltyMethod == '1' && array[i].payMethod == '1'){
                    delete array[i].gradientOne;
                    delete array[i].gradientTwo;
                    delete array[i].gradientThree;
                }else if(array[i].royaltyMethod == '1' && array[i].payMethod == '2'){
                    delete array[i].gradientTwo;
                    delete array[i].gradientThree;
                }else if(array[i].royaltyMethod == '2' && array[i].payMethod == '1'){
                    delete array[i].gradientOne;
                    delete array[i].gradientTwo;
                    delete array[i].gradientThree;
                }else if(array[i].royaltyMethod == '2' && array[i].payMethod == '2'){
                    delete array[i].gradientOne;
                    delete array[i].gradientThree;
                }else if(array[i].royaltyMethod == '3' && array[i].payMethod == '1'){
                    delete array[i].gradientOne;
                    delete array[i].gradientTwo;
                    delete array[i].gradientThree;
                }else if(array[i].royaltyMethod == '3' && array[i].payMethod == '2'){
                    delete array[i].gradientOne;
                    delete array[i].gradientTwo;
                }
            }
            let obj = {
                ...
                setSalaryCourseCommission : /*JSON.stringify(setSalaryCourseCommission)*/array
            };
                ...
                <QueueAnim
                    ...
                    { setSalaryCourseCommission && setSalaryCourseCommission.length > 0 ?
                        setSalaryCourseCommission.map(function(item,index){
                            return(
                                <div key = { item.zj_parent_index } className={styles.course_commission}>
                                    ...
                                    >
                                        {getFieldDecorator('course_' + item.zj_parent_index, {
                                            ...
                                            <Select
                                                ...
                                                onChange = {(currentId) => CourseSelectOnChange(currentId,getFieldValue('course_' + item.zj_parent_index))}
                                    ...
                                    { item.royaltyMethod == '1' && item.payMethod == '1' ?
                                        ...
                                        >
                                            {getFieldDecorator('1-1_commissionMoney_' + item.zj_parent_index, {
                                                initialValue : !isNaN(item.personMoney) ? item.personMoney + '' : undefined,
                                                rules : [
                                      ...
                                      item.royaltyMethod == '2' && item.payMethod == '1' ?
                                        ...
                                        >
                                            {getFieldDecorator('2_1_commissionMoney_' + item.zj_parent_index, {
                                                initialValue : !isNaN(item.courseMoney) ? item.courseMoney + '' : undefined,
                                                rules : [
                                                    { required : true , message : '请输入提成金额' , whitespace : true },
                                                    { validator: (rule, value, callback) => checkMoneyOrCourse(rule, value, callback,'提成金额') }
                                                ]
                                      ...
                                      item.royaltyMethod == '3' && item.payMethod == '1' ?
                                        ...
                                        >
                                            {getFieldDecorator('3_1_commissionRate_' + item.zj_parent_index, {
                                                initialValue : !isNaN(item.rate) ? item.rate + '' : undefined,
                                                rules : [
