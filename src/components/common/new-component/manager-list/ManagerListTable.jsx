	...
	return(
        <div className='zj_new_component_by_yhwu_table'>
            <Table
                ...
                scroll = {{ y : scrollHeight, x : !!xScroll && xScroll }}
                rowSelection = { rowSelection }
                locale = {{ emptyText : !!loading ? <ProgressBar content = { progressContent || '加载中'} height = '400px'/> : <NullData content = {
                        emptyText || '暂时没有数据'} height = { NullDataHeight || 400 }/> }}
                rowKey = { rowKey || 'id' }
            />
        </div>
	)
