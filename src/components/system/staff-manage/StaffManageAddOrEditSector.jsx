    ...
    function handleComplete(e) {
        ...
        validateFieldsAndScroll((errors,data) => {
            ...
            if(addOrEditSectorModalType == 'edit'){
                ...
                if(data.pids && data.pids.length > 0 && data.id == data.pids[data.pids.length - 1]){
                    return message.error('上级部门不可以是自身');
                }
            }
            if(data.pids && data.pids.length > 0){
                data.pids = data.pids[data.pids.length - 1];
            }else{
                data.pids = undefined;
            }
            ...
            <Form>
                ...
                >
                    {getFieldDecorator('pids', {
                        ...
                        <Cascader placeholder='请选择上级部门' options={allOrganList} changeOnSelect size='default' disabled = { addOrEditSectorModalType == 'edit' }/>
                    )}
                </FormItem>
                <FormItem
                    label="部门名称"
                ...
                >
                    {getFieldDecorator('departmentName', {
                        ...
                        <Input type="text" placeholder='请填写部门名称' size='default' onPressEnter = { handleComplete }/>
                    )}
                </FormItem>
            </Form>
        </Modal>
