...
function OrgLogo({
    ...
    let url = imgUrl == '' || imgUrl == null || imgUrl == undefined ? '//img.ishanshan.com/gimg/img/fa78332e0d89045a645a31c6f34ef223' : imgUrl
    let displayImg = [{
        uid : -1,
        url : url,
        name : 'orgLogo',
    ...
    return(
        <div style={{width:'100%',padding:'20px'}}>
            <Form horizontal className='zj_org_logo'>
                <FormItem
                    {...formItemLayout}
                >
            ...
            <div>
                <Button type='primary' style={{marginRight:'10px'}} loading={saveButtonLoading} onClick={() => UpdateOrgLogo(getFieldValue('imgUrl'))} >保存</Button>
                <Popconfirm placement="bottomLeft" title={<span>默认图将直接替换并保存，是否继续</span>} onConfirm={UseDefaultPic} okText="是" cancelText="否">
                    <Button type='ghost' loading={useDefaultButtonLoading} >使用默认图</Button>
                </Popconfirm>
            </div>
        </div>
    );
