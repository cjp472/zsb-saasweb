...
import style from './StaffManageChangeStatusFailedModal.less';
...
const StaffManageChangeStatusFailedModal = ({
    ...
    let paginationProps = {
        size : 'large',
        total: staffManageChangeStatusTableTotal,
        ...
        <Modal {...modalOpts}>
            { addOrEditStaffModalType != 'modifyFunction' ?
                <div style={{marginBottom:'16px',fontSize:'14px'}}>
                    <span style={{color:'#5d9cec'}}>共操作（{changeStatusOperateStaffNum}），</span>
                    <span style={{color:'#69b439'}}>成功（{changeStatusOperateStaffNum - staffManageChangeStatusTableTotal}），</span>
                    <span style={{color:'#ff4f3e'}}>失败（{staffManageChangeStatusTableTotal}）</span>
                </div>
                :
                null
            }
            <div style={{marginBottom:'16px',fontSize:'14px',overFlow:'hidden'}}>
