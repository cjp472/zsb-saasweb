...
function MicroModuleFormComponent ({
	...
	function toucAddActivity() {
		resetFields();
		dp('updateState', {showAddActivityModal : !showAddActivityModal, editStatus : false } );
		dp('summaryQuery', {} );
	}
	...
	function addActivityHandleCancel() {
		resetFields();
		dp('updateState', {showAddActivityModal : !showAddActivityModal, itemDataSource : {}, editStatus : !editStatus } );
	}

	//创建model完全关闭后状态
	function closeAddModal() {
		resetFields();
		dp('updateState', {saveButtonDsiabled : false, disabledCreateQrBtn : false, itemDataSource : {}} );
	}
	...
	function touchMemberCount(record) {
		dp('getQrCodeData', { showCreateQRModal: !showCreateQRModal, activityId: record.id, instanceOrgId: record.orgId} );
	}

	//编辑单条数据
	function editItemData(record){
		dp('selectItemData', {showAddActivityModal : !showAddActivityModal, record : record, editStatus : true} );
		dp('summaryQuery', { orgId : record.orgId} );
	}
            ...
            columns : [
                {
					...
					render    : ( text, record ) => (
						<div>
							<a onClick={() => editItemData(record)} disabled={record.isHq === 1}>编辑&nbsp;</a>
							<Popconfirm title="确定删除?" okText="是" cancelText="否" onConfirm={() => delectActivity(record)}>
								<a href="#" disabled={record.isHq === 1}>删除</a>
							</Popconfirm>
						</div>
					)
				},
	...
	qrData&&qrData.map((item, index) => {
		let qrLink = `${domain}html/market/activity?tenantId=${item.tenant_id}&orgId=${item.org_id}&activityId=${item.activity_id}&memberId=${item.id}&instanceOrgId=${instanceOrgId || ''}`;
	...
	let qrProps = {
		...
		activityId : qrData&&qrData.length > 0 ? qrData[0].activity_id : undefined,
	};
			  	...
			  	<div className={styles.modalButtonDivStyle}>
					<Button className={styles.modalButtonStyle} onClick={() => addActivityHandleCancel()}>取消</Button>
				  	<Button className={styles.modalButtonStyle}
						...
						disabled={editStatus ? (itemDataSource && String(itemDataSource.isHq) !== '1' ? saveButtonDsiabled : true) : disabledCreateQrBtn}
