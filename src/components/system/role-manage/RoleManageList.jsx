        ...
        childrenRoleList = allRoleList.map((item,index) => {
            ...
            if('admin' == item.roleKey){
                ...
                if( wetherRoleItemChooseIndex == index ){
                    return(
                        <div key={item.id} className={styles.roleNameCheck} onClick={() => CheckRoleFunction(item,index)}>
                    ...
                    return(
                        <div key={item.id} className={styles.roleNameUnCheck} onClick={() => CheckRoleFunction(item,index)}>
                            <span style={{marginLeft:'5px'}}>{item.name}(系统管理员)</span>
                            <a onClick={() => CopyRoleItem(item.id,item.name)} style={{float:'right',fontSize:'1.16rem',marginRight:'5px'}}>复制</a>
                        </div>
                    );
                        ...
                        return(
                            <div key={item.id} className={styles.roleNameCheck} onClick={() => CheckRoleFunction(item,index)}>
                                ...
                                <Popconfirm placement="top" title={<span>删除将导致该角色不可用，确定要删除<strong style={{color:'red'}}>{item.name}</strong>吗？</span>} onConfirm={() => DeleteRoleItem(item.id)} okText="是" cancelText="否">
                                    <a style={{float:'right',marginLeft:'20px',color:'#4e9ff8',fontSize:'14px',marginRight:'5px'}}>删除</a>
                                </Popconfirm>
                                <a onClick={() => CopyRoleItem(item.id,item.name)} style={{float:'right',marginLeft:'20px',fontSize:'14px'}}>复制</a>
                                <a onClick={() => RenameRoleName(item,index)} style={{float:'right',marginLeft:'20px',fontSize:'14px'}}>重命名</a>
                            </div>
                        );
                    }else{
                        return(
                            <div key={item.id} className={styles.roleNameUnCheck} onClick={() => CheckRoleFunction(item,index)}>
                                <span style={{marginLeft:'5px'}}>{item.name}</span>
                                <Popconfirm placement="top" title={<span>删除将导致该角色不可用，确定要删除<strong style={{color:'red'}}>{item.name}</strong>吗？</span>} onConfirm={() => DeleteRoleItem(item.id)} okText="是" cancelText="否">
                                    <a style={{float:'right',marginLeft:'20px',fontSize:'14px',marginRight:'5px'}}>删除</a>
                                </Popconfirm>
                                <a onClick={() => CopyRoleItem(item.id,item.name)} style={{float:'right',marginLeft:'20px',fontSize:'14px'}}>复制</a>
                                <a onClick={() => RenameRoleName(item,index)} style={{float:'right',marginLeft:'20px',fontSize:'14px'}}>重命名</a>
                            </div>
                        ...
                        <div key={item.id}>
                            <QueueAnim
                                ...
                                <div style={{height:'50px',lineHeight:'50px',borderBottom:'1px solid #f8f8f8'}} key='role_rename'>
                                    <Input type='text' placeholder={item.name} defaultValue={item.name} size='large' style={{width:'200px',top:'-1px',fontSize:'16px'}} onChange={CreateOrRenameOnChange}/>
                                    <a onClick={() => CancelRename()} style={{float:'right',marginLeft:'20px',fontSize:'14px',marginRight:'5px'}}>取消</a>
                                    <a onClick={() => RenameSubmit(item,index)} style={{float:'right',marginLeft:'20px',fontSize:'14px'}}>确定</a>
                                </div>
                            </QueueAnim>
                        </div>
                    )
    ...
    const loopAllFunctionList = data => data.map((item) => {
        if (item.list) {
            if(item.mainTitle == true){
                return <TreeNode title={<span style={{fontSize:'16px',color:'#666666',lineHeight:'20px'}}>{item.name}</span>} key={item.id} >{loopAllFunctionList(item.list)}</TreeNode>;
            }else{
                return <TreeNode title={<span style={{fontSize:'14px',color:'#999999',lineHeight:'18px'}}>{item.name}</span>} key={item.id} >{loopAllFunctionList(item.list)}</TreeNode>;
            }
        }else{
            return <TreeNode title={<span style={{fontSize:'14px',color:'#999999'}}>{item.name}</span>} key={item.id} />
        }
    });
                    ...
                    <Spin title='Loading' spinning={ allRoleListLoading }>
                        ...
                        <QueueAnim
                            ...
                            {createingRoleVisible ? [
                                <div style={{height:'50px',lineHeight:'50px',borderBottom:'1px solid #f8f8f8'}} key='create_new_role'>
                                    <Input type='text' size='large' style={{width:'200px',top:'-1px',fontSize:'16px'}} onChange={CreateOrRenameOnChange} placeholder='请输入新增角色名称' autoFocus/>
                                    <a onClick={() => CancelCreate()} style={{float:'right',marginLeft:'20px',fontSize:'14px',marginRight:'5px'}}>取消</a>
                                    <a onClick={() => CreateSubmit()} style={{float:'right',marginLeft:'20px',fontSize:'14px'}}>确定</a>
                                </div>
                            ]:null}
                        </QueueAnim>
                    </Spin>
                </div>
                <Button type='primary' onClick={CreateRole} disabled={createingRoleVisible == true ? true : false} style = {{ float:'right' }}>新增</Button>
            </div>
