...
export default {
        ...
        setup({ dispatch, history }) {
            history.listen(location => {
                if (location.pathname === '/sys_org_list') {
					...
					dispatch({
						type : 'OpenAddOrgModalGetCheckBox',
						payload : {
							organId : window._init_data.cerp_orgId
						}
					})
                }
            });
    ...
    effects: {
        ...
        *'EditExistOrg'({ payload },{ put , call , select }){
            yield put({ type : 'showTableLoading' });
			let imgLogo = yield call( saveOrgImg, ({ imgurl : payload.schoolLogo }) );
			if( imgLogo  && imgLogo.ret && imgLogo.ret.errorCode == 9000 ){
				/*saas与招生宝合并之后的获取机构图片*/
				yield put({
				...
				message.error( imgLogo && imgLogo.ret && imgLogo.ret.errorMessage || '校区logo设置失败' );
			}
